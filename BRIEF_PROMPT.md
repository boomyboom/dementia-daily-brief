...
4. 중복과 낡은 자료를 방지한다.
   ...
   - **신선도(중요)**: 각 항목은 게재 전 반드시 **원문 URL을 WebFetch로 열어 실제 발행일을 직접 확인**한다. 페이지에 적힌 날짜 기준으로 오늘로부터 **7일 이상 지났으면 제외**한다. 날짜를 **추측·창작하지 말 것** — 발행일을 확인할 수 없으면 그 항목은 넣지 않는다. (보도자료·회사 페이지는 오래된 글이 검색에 잡히는 경우가 많으니 특히 주의. 예: 수년 전 press release가 최신처럼 보일 수 있음.)
   - 정부계획·제도처럼 "새 소식이 아니라 기존 사실의 재보도"인 경우도 제외한다. 억지로 채우지 말고, 신규·검증된 소식이 없는 섹션은 빈 배열로 둔다.
5. 웹 검색(한국어+영어)으로 6개 섹션의 최신 소식을 수집한다. 섹션별로 Task 서브에이전트를 병렬 실행하면 효율적이다. (저널 목록·분류 기준은 BRIEFING_GUIDE.md 참조. 모든 논문은 치매/AD 관련만.)
   - research (진단·영상 연구): 뇌영상(MRI/PET) AI, 진단·예후 모델, 진단 바이오마커, 진단정확도 — BeauBrain 직결
   - research_broad (치매 전분야 연구): 치료제·기전·유전·역학·예방·디지털/인지평가 등 치매·AD 전반
   - regulatory: FDA 인허가, 식약처 허가, NECA 신의료기술·평가유예·혁신의료기술 고시, 심평원 수가, **국가 보건의료 정책·전략 문서(관계부처 합동/복지부/과기정통부 전략·계획, 예: AI 기본의료 전략) — 신규·개정 시 회사 관련 부분만 발췌** (BRIEFING_GUIDE.md의 회사 관련성 필터 참조)
