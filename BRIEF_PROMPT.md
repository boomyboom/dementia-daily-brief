치매·알츠하이머병 데일리 브리프를 생성하고 GitHub Pages 사이트를 갱신하라. 이 작업은 사람이 보지 않는 자동 실행이므로, 막히면 멈추지 말고 합리적으로 판단해 끝까지 진행한다.

## 저장소
- 로컬 경로: /Applications/BeauBrain/700_Utils/004_DAILY_BRIEF (이미 이 디렉토리에서 실행 중)
- 원격: https://gitlab.com/beaubrainsbpark/dementia-daily-brief (main 브랜치, 토큰 인증됨)
- 사이트: https://beaubrainsbpark.gitlab.io/dementia-daily-brief/

## 절차
1. `git pull`로 최신 상태를 받는다.
2. 오늘 날짜(KST, `date +%Y-%m-%d`)를 확인하고, 실행 모드를 판단한다.
   - **신규 모드 (오늘 파일 없음, 보통 오전 첫 실행)**: 아래 절차대로 오늘 브리핑을 새로 생성한다.
   - **갱신 모드 (오늘 파일 이미 있음, 보통 오후 실행)**: 기존 `briefs/오늘날짜.json`을 읽어 이미 실린 항목을 모두 파악한 뒤, 그 이후 새로 공개된 검증된 소식만 찾아 **해당 섹션에 추가(병합)**한다. 기존 항목은 그대로 두고, 이미 실린 것·이전 브리핑과 중복되는 것은 절대 추가하지 않는다. 새로 추가할 검증된 소식이 하나도 없으면 파일을 바꾸지 말고 그대로 종료한다(빈 커밋 금지).
3. `BRIEFING_GUIDE.md`를 읽고 그 규칙을 정확히 따른다. 수집 대상 섹션(research/regulatory/market/competitors/media), 경쟁사 추적 목록, 품질 규칙, JSON 포맷이 모두 정의되어 있다.
4. 중복과 낡은 자료를 방지한다.
   - `briefs/seen_urls.json`(지금까지 게재한 모든 원문 URL 목록)을 읽는다. **이 목록에 이미 있는 URL은 절대 다시 게재하지 않는다(하드 차단).**
   - 추가로 `briefs/` 폴더의 최근 브리핑 파일 5~7개를 읽어, **URL이 달라도 같은 사건·같은 논문·같은 정책을 다룬 항목은 중복으로 보고 제외**한다. (예: "제5차 치매관리종합계획", 특정 제품의 같은 인허가 등 — 다른 매체가 재보도해도 이미 다뤘으면 넣지 않는다.)
   - **신선도(중요)**: 각 항목은 게재 전 반드시 **원문 URL을 WebFetch로 열어 실제 발행일을 직접 확인**한다. 페이지에 적힌 날짜 기준으로 오늘로부터 **7일 이상 지났으면 제외**한다. 날짜를 **추측·창작하지 말 것** — 발행일을 확인할 수 없으면 그 항목은 넣지 않는다. (보도자료·회사 페이지는 오래된 글이 검색에 잡히는 경우가 많으니 특히 주의. 예: 수년 전 press release가 최신처럼 보일 수 있음.)
   - **단순 재보도는 제외하되, 새로운 정보가 있는 후속 기사는 허용한다.**
     - 제외: 이미 다룬 사건을 다른 매체가 사실관계만 똑같이 옮긴 기사, 오래된 정부계획을 다시 소개하는 기사.
     - **허용**: 이미 다룬 사건이라도 **새로운 분석·업계 관점·수치·이해관계자 반응·제도적 함의**를 담은 심층/기획 기사(예: 히트뉴스·청년의사의 규제 이슈 분석, 시장 파급 해설). 이 경우 요약에 "무엇이 새로 밝혀졌는지"를 중심으로 쓰고, 이미 아는 사실 반복은 생략한다. 규제·시장 섹션에서는 원 사건보다 이런 해설이 더 유용할 때가 많으니 지나치게 배제하지 말 것.
   - 억지로 채우지 말고, 신규·검증된 소식이 없는 섹션은 빈 배열로 둔다.
5. 웹 검색(한국어+영어)으로 6개 섹션의 최신 소식을 수집한다. 섹션별로 Task 서브에이전트를 병렬 실행하면 효율적이다. (저널 목록·분류 기준은 BRIEFING_GUIDE.md 참조. 모든 논문은 치매/AD 관련만.)
   - research (진단·영상 연구): 뇌영상(MRI/PET) AI, 진단·예후 모델, 진단 바이오마커, 진단정확도 — BeauBrain 직결
   - research_broad (치매 전분야 연구): 치료제·기전·유전·역학·예방·디지털/인지평가 등 치매·AD 전반
   - regulatory: FDA 인허가, 식약처 허가, NECA 신의료기술·평가유예·혁신의료기술 고시, 심평원 수가, **국가 보건의료 정책·전략 문서(관계부처 합동/복지부/과기정통부 전략·계획, 예: AI 기본의료 전략) — 신규·개정 시 회사 관련 부분만 발췌** (BRIEFING_GUIDE.md의 회사 관련성 필터 참조)
     - **1차 출처 직접 확인 의무**: 언론 검색에만 의존하지 말고, 매 실행마다 아래 공식 페이지를 **WebFetch로 최소 1회씩 직접 열어** 최근 공고·보도자료를 확인한다. (언론에 안 실린 공고가 자주 있음)
       · 식약처 보도자료 https://mfds.go.kr/brd/m_99/list.do
       · NECA 알림·공지 https://neca.re.kr
       · 심평원 보도자료 https://www.hira.or.kr
       · 보건복지부 보도자료 https://www.mohw.go.kr/board.es?mid=a10503000000&bid=0027
       확인 결과 관련 신규 건이 없으면 "1차 출처 확인함, 해당 없음"이라고 로그에 남기고 넘어간다.
   - market: 시장 리포트, 투자·M&A, 치료제(레켐비/키순라) 시장 동향
   - competitors: BRIEFING_GUIDE.md의 추적 목록 기업 동향 (뉴로핏, 뷰노, 제이엘케이, 휴런, 아이메디신, 딥노이드 등)
     - **의료 전문지와 경제지를 모두 검색할 것.** IPO·기술성평가·투자유치·유상증자·실적은 의료지에 안 실리고 **서울경제·더벨·한국경제·머니투데이·이데일리·DART 공시**에만 나온다. 회사명+자본이벤트 조합으로 별도 검색한다(예: `"휴런" IPO`, `"뉴로핏" 투자유치`).
     - 경쟁사 섹션이 0건으로 나오면 경제지 검색을 실제로 했는지 한 번 더 확인한다.
   - media: Alzforum/Fierce Biotech/국내 의료전문지 주요 보도
6. 검증된 항목으로 `briefs/오늘날짜.json`을 BRIEFING_GUIDE.md의 JSON 포맷대로 작성한다. 모든 항목에 실제 원문 URL 필수. 섹션에 소식이 없으면 빈 배열로 둔다.
   - 신규 모드: headline은 그날 가장 중요한 소식 한 문장으로 새로 작성한다.
   - 갱신 모드: 기존 항목을 유지한 채 새 항목만 각 섹션에 덧붙인다. headline은 기존 것을 유지하되, 새로 추가된 소식이 그날 가장 중요하다면 그때만 교체한다.
7. `python3 cleanup_old_briefs.py`를 실행한다. 이 스크립트가 30일 지난 브리핑 파일을 삭제하고, `briefs/manifest.json`(날짜 목록)과 `briefs/seen_urls.json`(중복 차단용 URL 목록)을 남은 브리핑 기준으로 자동 재생성한다. (manifest·seen_urls를 손으로 고칠 필요 없음)
   이어서 `python3 build_keywords.py`를 실행해 `briefs/keywords.json`(30일 키워드·기업 랭킹, 전일 대비 등락)을 갱신한다. 사이트 오른쪽 랭킹 패널이 이 파일을 읽는다.
8. **링크 안전성**: 모든 `url`은 반드시 `https://`로 적는다(평문 `http://` 금지 — 브라우저·보안SW가 경고를 띄운다). 원문이 http만 제공하면 https로 접속되는지 확인하고, 안 되면 그 항목은 넣지 않는다.
   이어서 `python3 check_links.py`를 실행해 평문HTTP·타도메인 이동·삭제된 링크를 점검한다. 문제가 나오면 살아있는 대체 출처로 교체하거나 해당 항목을 제외한다.
9. `python3 -m json.tool`로 오늘 브리핑·manifest.json·seen_urls.json의 유효성을 검증한다.
10. `git add -A`로 변경(신규·삭제 포함)을 스테이징한 뒤 **GitLab과 GitHub 양쪽에 push**한다: `git push gitlab main` 그리고 `git push origin main`. (한쪽이 실패해도 다른 쪽은 시도한다.) 커밋 메시지는 신규 모드면 `brief: YYYY-MM-DD 데일리 브리프 추가`, 갱신 모드면 `brief: YYYY-MM-DD 갱신 (추가 N건)`으로 한다. index.html 등 사이트 코드는 수정하지 않는다.

## 성공 기준
- briefs/오늘날짜.json이 유효한 JSON으로 생성·푸시됨
- 모든 항목이 최근 소식이고 원문 URL이 있으며 이전 브리핑과 중복되지 않음
- BeauBrain(뇌 MRI 기반 치매 진단 SW) 사업 관련성이 높은 소식에 importance high/medium이 정확히 매겨짐
